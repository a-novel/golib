...
type Formatter interface {
	...
	Log(content LogContent, level logger.LogLevel)
}
...
func (f *consoleFormatterImpl) Log(content LogContent, level logger.LogLevel) {
	...
	if !f.static {
		...
		if isDynamic {
			dynamic.RenderConsoleDynamic(func(msg string) { f.logger.Log(level, msg) })
			return
		}
	}

	f.logger.Log(level, content.RenderConsole())
}
...
func NewConsoleFormatter(consoleLogger logger.Console, static bool) Formatter {
...
// Log implements the Formatter.Log interface.
func (f *jsonFormatterImpl) Log(content LogContent, level logger.LogLevel) {
	f.logger.Log(level, content.RenderJSON())
}

// NewJSONFormatter creates a new Formatter that will dispatch messages to a logger.JSON.
func NewJSONFormatter(jsonLogger logger.JSON) Formatter {
