...
// Taken from there because they made it fucking internal
// https://github.com/grpc/grpc-go/blob/master/internal/stubserver/stubserver.go
...
func StartTestService(t *testing.T, server *StubServer, sopts ...grpc.ServerOption) *StubServer {
	if server == nil {
		server = &StubServer{
			...
			UnaryCallF: func(context.Context, *testgrpc.SimpleRequest) (*testgrpc.SimpleResponse, error) {
				return &testgrpc.SimpleResponse{}, nil
			},
		}
	}
	_ = server.StartServer(sopts...)
